...
import { savePackageBookingAPI } from "../../api/package";
...
const PackageBooking = () => {
    ...
    const handleOpenDialog = (message = "Package Booking Success", icon) => {
        setOpenDialog(true);
    ...
    const handleCheckout = () => {
        if (formData.customer_first_name === "") {
            ...
            try {
                ...
                setPackageBookingData(updatedFormData);
            } catch (error) {
    ...
    useEffect(() => {
        const savePackageBooking = () => {
            savePackageBookingAPI(packageBookingData)
                .then((res) => {
                    if (res.data.length !== 0 && res.status === 200) {
                        setIsSuccess('success');
                        handleOpenDialog(
                            <>
                                Package Booking Success! <br />
                ...
                .catch((error) => {
                    console.error("Error during package booking API request:", error);
                    handleOpenDialog(
                        "Something went wrong.",
                        <ErrorSharpIcon style={{ color: "red", fontSize: "40px" }} />
                    );
    ...
    return (
        <React.Fragment>
            ...
            <div className="packageBookingCard">
                <Card />
                                    ...
                                    <TableBody>
                                        {cartData.map((row, index) => (
                                            <TableRow key={index}>
                                                ...
                                                <TableCell align="center">{row.number_of_participants}</TableCell>
